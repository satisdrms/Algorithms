...
public class Topic_19_Sort_EX7_BookSolutionYetToBeWorked1 {
	...
	private static void findTower(int[][] p) {
		...
		while (current < p.length) {
			...
			for (int i = current; i < p.length; i++) {
			 ...
			 System.out.println("f " + firstUnfit + " prevL1 " + prevL1+" first fit " + firstFit);
			if (maxSeq.size() == 0)
				maxSeq = next;
			if (current != firstUnfit) {
				...
				for (Integer in : path) {
					...
					if (maxSeq.size() < nextseq.size()) {
						...
						firstFit = getLastFitBelowCurrentUnFit(unFitItems, in);
					}
				}
			...
			prevL1 = firstFit;
			 System.out.println("Current is "+current+" and prev is "+prevL1);
			 printArray(maxSeq);
	...
	private static int getLastFitBelowCurrentUnFit(
			LinkedHashSet<Integer> unFitItems, Integer in) {
		int lastFit = 0;
		for (int i = 0; i < in; i++) {
			if (!unFitItems.contains(i)) {
				lastFit = i;
			}
		}
		return lastFit;
