 ...
 */
public class Topic_11_Working_2_Heap_Comparator<T> {
	private int size;
	 ...
	 */
	public Topic_11_Working_2_Heap_Comparator(Comparator<T> comparator) {
		size = 0;
		// Allocate space
	...
	private final boolean decreaseKey(final Node node) {
		...
		// ) {
		while (index > 0
				&& comparator.compare(heap.get(parent(index)).element,
	...
	private final void minHeapify(final Node node) {
		...
		if (right < size
				&& comparator.compare(heap.get(right).element,
						heap.get(smallest).element) <= 0)
			smallest = right;
		if (smallest != index) {
...
// 1. Demonstrates heaps Demonstrates heaps
// 2. Fibonacci heap data structure
// 3. Binary Heap Queue
// 4. Tree Heap
// 5. This class implements the heap interface using a java.util.List as the
// underlying data structure.
// 6. A heap-based priority queue, without any concurrency control A heap-based
// priority queue, without any concurrency control
// 7. A MinMaxHeap provides a heap-like data structure that provides fast access
// to both the minimum and maximum elements of the heap.
